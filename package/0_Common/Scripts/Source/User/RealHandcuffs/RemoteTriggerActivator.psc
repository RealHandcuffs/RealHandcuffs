...
Function TriggerVanillaShockCollars(ObjectReference[] candidateActors)
    Int index = 0
    While (index < candidateActors.Length)
        Actor target = candidateActors[index] as Actor
        If (target != None && !target.HasKeyword(Library.Resources.RemoteTriggerEffect) && Library.SoftDependencies.IsWearingVanillaShockCollar(target))
            Var[] kArgs = new Var[1]
            kArgs[0] = target
            Library.SoftDependencies.CallFunctionNoWait("TriggerVanillaShockCollar", kArgs)
        EndIf
        index += 1
    EndWhile
EndFunction