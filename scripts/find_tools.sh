...
# this script looks for the required tools and sets env variables that point to them:
...
do
  case "$VAR" in
    "-q" )
      QUIET=1;;
    "-h" )
      echo "Usage: $(basename "$0") [-q]" && exit -1;;
    * )
      echo "Invalid argument: $VAR" && exit -1;;
  esac
done
...
then
  ...
  then
    ...
    [[ $QUIET == 0 ]] && echo "Fallout 4: $DIR_FALLOUT4"
  fi
fi
...
then
  [[ $QUIET == 0 ]] && echo "Fallout 4 Creation Kit: In tools directory."
  ...
  then
    [[ $QUIET == 0 ]] && echo "  Resolved to: $DIR_FALLOUT4CREATIONKIT"
  fi
else
  if [ -f "$DIR_FALLOUT4/CreationKit.exe" ]
  then
    DIR_FALLOUT4CREATIONKIT="$DIR_FALLOUT4"
    [[ $QUIET == 0 ]] && echo "Fallout 4 Creation Kit: In Fallout 4 directory."
  fi
fi
